...
start_wc() {
	logger -p daemon.info -t "ModemManager[$$]" "Starting watchcat"
...
proto_modemmanager_init_config() {
	...
	proto_config_add_boolean autoapn
...
proto_modemmanager_setup() {
	...
	local device apn autoapn allowedauth username password pincode iptype metric signalrate
	...
	mmcli --modem="${device}" --timeout 130 --simple-connect="${connectargs}" || {
