...
get_ping_size() {
	...
	jumbo)
		...
		;;
	*)
		logger -p daemon.error -t "watchcat[$$]" "Error: invalid ping_size. ping_size should be either: small, windows, standard, big, huge or jumbo" 1>&2
		logger -p daemon.error -t "watchcat[$$]" "Corresponding ping packet sizes (bytes): small=1, windows=32, standard=56, big=248, huge=1492, jumbo=9000" 1>&2
		;;
	esac
	echo $ps
}
...
get_ping_family_flag() {
	...
	ipv6)
		...
		;;
	*)
		logger -p daemon.error -t "watchcat[$$]" "Error: invalid address_family \"$family\". address_family should be one of: any, ipv4, ipv6" 1>&2
		;;
	esac
	echo $family
}
...
run_script)
	...
	;;
*)
	logger -p daemon.error -t "watchcat[$$]" "Error: invalid mode selected: $mode"
	;;
esac
